...
public class ApplyHazardOnCMHandler extends CmdHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (eo instanceof Element) {
			...
			ArrayList<Element> commMediasList = new ArrayList<Element>();

			if (UMLUtil.getStereotypeApplication(element, CRA.class) != null) {
				nacreModelerHelper.getCMfromCRA(element, commMediasList);
			}
			...
			// package)
			if (commMediasList != null && !commMediasList.isEmpty()) {
				Element communicationMedia = commMediasList.get(0);
				Stereotype stereotype = (communicationMedia).getApplicableStereotype("hazards::CommunicationHazard");
				CompoundCommand compoundCommand = new CompoundCommand();
				for (Element comMedia : commMediasList) {
